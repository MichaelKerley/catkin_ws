...
namespace avoidance
{
  ...
  SafeLandingPlannerNode::SafeLandingPlannerNode(const ros::NodeHandle &nh) : nh_(nh), spin_dt_(0.1)
  {
    ...
    //sub scribe to local position from mavros
    //call back in this file
    ...
    //subscribe to pointcloud from camera topic 
    //call back in this file
  ...
  //poitionCallback for /mavros/local_position/pose topic
  ...
  //helper method
  //start node called in main defined here
  void SafeLandingPlannerNode::startNode()
  {
    ...
    ros::TimerOptions timer_options(ros::Duration(spin_dt_), boost::bind(&SafeLandingPlannerNode::cmdLoopCallback, this, _1), &cmdloop_queue_);
  ...
  void SafeLandingPlannerNode::cmdLoopCallback(const ros::TimerEvent &event)
  {
    ...
    safe_landing_planner_->setPose(avoidance::toEigen(current_pose_.pose.position), avoidance::toEigen(current_pose_.pose.orientation));

    {
      std::lock_guard<std::mutex> transformed_cloud_guard(*(transformed_cloud_mutex_));

      //runSafeLandingPlanner is defined in safe_landing_planner
      safe_landing_planner_->runSafeLandingPlanner();
      cloud_transformed_ = false;
    }
    visualizer_.visualizeSafeLandingPlanner(*(safe_landing_planner_.get()), current_pose_.pose.position, previous_pose_.pose.position, rqt_param_config_);
    publishSerialGrid();
